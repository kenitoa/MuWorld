...
internal sealed class PlayerProgress
{
    ...
    public int TotalGreatCount { get; set; }
    public int TotalBetterCount { get; set; }
    public int TotalExactGoodCount { get; set; }
    ...
    public HashSet<string> ActivePlayDateKeys { get; set; } = [];
...
internal static class AchievementCatalog
{
    ...
    public static List<AchievementDefinition> ApplySession(PlayerProgress progress, GameSessionSummary session)
    {
        ...
        progress.TotalGreatCount += session.GreatCount;
        progress.TotalBetterCount += session.BetterCount;
        progress.TotalExactGoodCount += session.GoodCount;
        progress.TotalBadCount += session.BadCount;
        progress.TotalMissCount += session.MissCount;
        progress.TotalNotesProcessed += session.TotalJudgedNotes;
        if (session.DurationSeconds > 0f)
            progress.TotalPlaySeconds += Math.Max(0, (int)MathF.Round(session.DurationSeconds));
        string dateKey = string.IsNullOrWhiteSpace(session.PlayedDateKey)
            ? DateTime.Now.ToString("yyyy-MM-dd")
            : session.PlayedDateKey;
        progress.ActivePlayDateKeys.Add(dateKey);
        string gradeKey = ScoreManager.FormatGrade(session.Grade);
        progress.GradeCounts[gradeKey] = progress.GradeCounts.GetValueOrDefault(gradeKey) + 1;
