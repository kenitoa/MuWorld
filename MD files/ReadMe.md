...
`run.bat`는 저장소 안의 `dotnet\dotnet.exe`를 우선 사용합니다. SDK가 없으면 .NET 9 SDK 설치를 시도한 뒤 Release 빌드 결과인 `bin\Release\net9.0-windows\game start.exe`를 실행합니다.

개발용으로 직접 빌드하려면 다음 명령을 사용합니다.

```powershell
.\dotnet\dotnet.exe build .\RhythmGame.csproj -c Debug
.\dotnet\dotnet.exe run --project .\RhythmGame.csproj -c Debug
```
...
곡은 `Songs\InGameBGM\Original` 폴더에서 읽습니다. 지원하는 오디오 확장자는 다음과 같습니다.
...
곡 옆에 같은 이름의 JSON 파일을 두면 제목, 아티스트, BPM, 미리듣기 구간, 커버 이미지, BGA 이미지 같은 메타데이터를 지정할 수 있습니다. 자세한 형식은 [docs/song-sidecar-schema.md](docs/song-sidecar-schema.md)를 참고하세요.
...
MuWorld는 BMS subset 형식의 차트를 사용합니다. 차트 규칙은 [docs/chart-format.md](docs/chart-format.md)에 정리되어 있습니다.
...
테스트는 점수 계산, BMS 파싱, 차트 생성, 설정 복구, 곡 기록, 통계, 실제 곡 파일 탐색, 판정 타이밍, Long/Slide 노트, 레인 전환, UI 렌더링, 설정 상호작용, 장시간 엔진 시뮬레이션을 확인합니다.

## 문제 해결

- 실행 파일이 빌드되지 않으면 실행 중인 `game start.exe`를 닫고 다시 빌드하세요.
- 곡이 보이지 않으면 `Songs\InGameBGM\Original`에 오디오 파일이 있는지 확인한 뒤 Song Select에서 rescan을 실행하세요.
